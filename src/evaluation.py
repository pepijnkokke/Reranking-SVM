...
def best_reranking(inputs, candidates, references, classifier, normalizer, pca, limit=1000, improve_limit=10):
    ...
    for i, input in enumerate(inputs):
        ...
        if i > 10:
            if blue_diff < -0.028:
                print("\rReranking broken off, BLEU diff to far down %0.5f" % blue_diff)
                break
    ...
    print("\rReranking finished classifications: %d" % classifications)
...
def print_evaluation(inputs, references, candidates, classifier, normalizer, pca, limit=1000, improve_limit=10):
    ...
    bleu_hypotheses_reranking = best_reranking(inputs, candidates, bleu_references, classifier, normalizer, pca, limit, improve_limit)
